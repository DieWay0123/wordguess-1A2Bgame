...
class ServerGUI:
    ...
    def check_client_guess(self, guess: str):
        ...
        if A == self.answer_length: # 猜到正確答案回傳答對的封包給client
            return f"[Success]: 恭喜猜對了!{A}A{B}B"
        return f"{A}A{B}B"
        ...
        while self.running:
            try:
                ...
                # 當client發送packet為guess相關時，進行答案判斷
                    ...
                    self.server_socket.sendto(server_reply.encode(), self.client_address)
                # 猜對後，處理client遊戲資料:
    ...
    def stop_server(self):
        ...
        if self.server_socket:
            ...
            self.output_text.insert(tk.END, "[Info]: 伺服器已關閉。\n")
    ...
    def set_answer(self):
        ...
        self.modify_output_text(f"[Info]: ✅正確答案已設定為：{answer}\n")
    ...
    def add_user_rankings(self, username, guess_count, duration, finish_time_str):
        ...
        for i, record in enumerate(self.rankings, 1):
            self.modify_output_text(
                f"{i}. {record['name']} - {record['guesses']} 次, {record['time']} 秒\n"
            )
    # 用來獲取目前遊戲使用者為第幾名
    ...
    def show_rankings(self, highlight_username=None, highlight_time=None, highlight_finish=None):
        ...
        for i, rec in enumerate(self.rankings, 1):
            ...
            if is_highlight:
                self.output_text.config(state="normal")
                self.output_text.insert(tk.END, line, "bold") #設定為粗體字
                self.output_text.config(state="disabled")
            else:
